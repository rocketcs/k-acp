        ...
        <article v-for="message in displayMessages" :key="message.id" :class="message.role === 'user' ? 'user-message' : 'answer'" @click="message.role === 'assistant' && (selectedAssistantMessageId = String(message.id))">
          ...
          <template v-else>
            ...
            <MarkdownRenderer
              ...
              :content="splitAssistantContent(message.content).before"
              :is-streaming="isRunning && selectedTurn === String(message.id)"
              :is-diy-chat="true"
            />
            <p v-else-if="shouldRenderAssistantPlaceholder({ hasContent: Boolean(splitAssistantContent(message.content).before), hasEvidence: Boolean(activeEvidence) })" class="answer-text">正在整理查询结果…</p>
            <template v-if="selectedTurn === String(message.id) && activeEvidence">
              <div v-if="activeEvidence.result.rows.length" class="result-table">
                <table>
                  ...
                  <tbody><tr v-for="(row, index) in activeEvidence.result.rows" :key="index"><td v-for="column in resultColumns" :key="column.key" :title="column.formatValue(row[column.key])">{{ column.formatValue(row[column.key]) ?? '-' }}</td></tr></tbody>
                </table>
              </div>
              <p v-else class="inline-state">本次查询未返回业务记录。</p>
            </template>
            <p v-if="selectedTurn === String(message.id) && activeOutcome" class="state-error">{{ activeOutcome.status === 'blocked' ? '该查询已被安全规则拦截，未执行任何数据操作。' : '语义依据暂不可用，请稍后重试。' }} {{ activeOutcome.reason || '' }} {{ activeOutcome.trace_id ? `Trace: ${activeOutcome.trace_id}` : '' }}</p>
            <MarkdownRenderer
              v-if="splitAssistantContent(message.content).after"
              class="answer-text markdown-answer"
              :content="splitAssistantContent(message.content).after"
              :is-streaming="isRunning && selectedTurn === String(message.id)"
              :is-diy-chat="true"
            />
            <footer v-if="selectedTurn === String(message.id) && activeEvidence"><button class="evidence-link" @click.stop="focusGraph"><ShareAltOutlined /> 查看语义依据</button><span>Trace: {{ activeEvidence.trace_id }}{{ activeEvidence.result.truncated ? ' · 结果已截断' : '' }}</span></footer>
          </template>
        </article>
