...
// 南网域：库存记录/资料来源是真实业务与溯源节点，不是查询过程节点。
const QUERY_PROCESS_KINDS: Set<GraphifyEvidenceNode['kind']> = new Set()
...
function nodeVisible(node: GraphifyEvidenceNode, opts: GraphModelOptions): boolean {
  ...
  // 南网域 entity（仓库/供应商/项目/区域）为核心业务对象，full 视图始终可见
  if (node.kind === 'entity') return opts.viewMode === 'full'
  return true
}
