...
import { splitChartSegments } from './chartSpec.ts'
...
/** 方案 A：```chart 围栏 → 受限 spec → ChartCard；无围栏时保持既有渲染路径。 */
const chartSegments = computed(() => splitChartSegments(props.content ?? ''))
const hasChartSegments = computed(() => chartSegments.value.some((segment) => segment.kind !== 'md'))
...
/** 正文是否已含 Markdown 表格（LLM 自行输出时不再重复渲染平台表） */
const hasMarkdownTable = computed(() => /\n\|/.test(props.content ?? '') || /^\|/.test((props.content ?? '').trim()))
/**
 * 平台结果表兜底：查询有真实数据但 LLM 未输出 Markdown 表格时
 * （巨型结果流式截断/叙述遗漏等），直接以执行器结果渲染紧凑表，保证"查询必有数据"。
 */
const shouldRenderCompactTable = computed(() => Boolean(props.evidence && hasRows.value && !hasMarkdownTable.value && !props.isStreaming))
const compactColumns = computed(() => props.evidence ? buildResultColumns(props.evidence, props.evidence.semantic_context.domain_labels) : [])
...
<template>
  <div class="graphify-assistant-message">
    <template v-if="hasChartSegments">
      <template v-for="(segment, index) in chartSegments" :key="index">
        <MarkdownRenderer
          v-if="segment.kind === 'md'"
          class="graphify-assistant-text"
          :content="segment.text"
          :is-streaming="isStreaming"
          :is-diy-chat="true"
        />
        <ChartCard v-else-if="segment.kind === 'chart'" :spec="segment.spec" />
        <p v-else class="graphify-inline-state">图表生成中…</p>
      </template>
    </template>
    <template v-else>
    <MarkdownRenderer
    ...
    <!-- 平台结果表兜底：查询有数据但正文缺表格时，用执行器结果保证数据可见 -->
    <div v-if="shouldRenderCompactTable" class="graphify-compact-table" :class="{ standalone: !bodyText && !afterTableText }">
      <table>
        <thead>
          <tr>
            <th v-for="column in compactColumns" :key="column.key">{{ column.label }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, index) in evidence?.result.rows" :key="index">
            <td v-for="column in compactColumns" :key="column.key"
              :title="column.formatValue(row[column.key])">{{ column.formatValue(row[column.key]) || '-' }}</td>
          </tr>
        </tbody>
      </table>
      <p v-if="evidence?.result.truncated" class="graphify-compact-note">结果较多，已截断展示。</p>
    </div>
...
.graphify-assistant-text {
  ...
  /* LLM 输出的 Markdown 表格同样紧凑化：一屏可见更多行 */
  :deep(table) {
    display: block;
    max-height: 400px;
    overflow: auto;
...
/* 平台兜底紧凑表：固定表头 + 限高滚动，行高压缩 */
.graphify-compact-table {
  margin-top: 12px;
  max-height: 400px;
  overflow: auto;
  ...
  th {
    ...
    color: #35578a;
    font-weight: 600;
    box-shadow: 0 1px 0 rgba(47, 100, 190, 0.2);
  }
