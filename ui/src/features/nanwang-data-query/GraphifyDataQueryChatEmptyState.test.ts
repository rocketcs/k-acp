...
test('query progress follows the latest user message and graphify alone can keep the finished card in place', () => {
  ...
  assert.match(chatStream, /const completedToolName = activeTool\?\.name \?\? toolNameById\.get\(e\.toolCallId\) \?\? ''/)
  assert.match(chatStream, /toolName: completedToolName, status: 'completed', content: e\.content \}\)/)
  assert.match(chatStream, /const failedToolName = failedTool\?\.name \?\? \(failedTool \? toolNameById\.get\(failedTool\.id\) \?\? '' : ''\)/)
  assert.match(chatStream, /toolName: failedToolName, status: 'failed' \}\)/)
  assert.match(chatStream, /toolNameById\.set\(e\.toolCallId, e\.toolCallName\)/)
  assert.match(chatStream, /toolNameById\.clear\(\)/)
