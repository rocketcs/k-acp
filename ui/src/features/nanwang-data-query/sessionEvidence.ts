...
import { resolveGraphExplorerApiBase } from './graphExplorer.ts'
...
async function fetchEvidenceEnvelope(graphRef: string): Promise<GraphifyEvidenceEnvelope | null> {
  try {
    const base = resolveGraphExplorerApiBase(import.meta.env.VITE_NANWANG_GRAPH_EXPLORER_API_BASE)
    const response = await fetch(`${base.replace(/\/$/, '')}/evidence/${encodeURIComponent(graphRef)}`)
