...
const GRAPHIFY_DATA_QUERY_AGENT_CODE = 'default-nanwang-data-query'
...
/** 工具返回结构化失败后，收口所有未执行的查询阶段，避免进度卡出现虚假的“等待”。 */
const queryFlowHasFailure = ref(false)
...
/** 事件乱序/丢失兑底：后面步骤完成时，把前面仍挂起的预置步骤收口为完成，避免进度条倒挂。 */
function settleEarlierSteps(currentStepName: string) {
  const currentIndex = QUERY_FLOW_STEPS.findIndex((step) => step.name === currentStepName)
  if (currentIndex <= 0) return
  const now = Date.now()
  for (const activity of queryFlowActivities.value) {
    const idx = QUERY_FLOW_STEPS.findIndex((s) => s.name === activity.name)
    if (idx >= 0 && idx < currentIndex && activity.status !== 'completed' && activity.status !== 'failed') {
...
function isFailedToolResult(content?: string): boolean {
  ...
  try {
    ...
    return ['blocked', 'unavailable', 'error', 'failed'].includes(status)
  } catch {
    return /MCP service .*unavailable|元模型库加载失败|服务内部错误|DM8 执行失败/i.test(content)
  }
}

function failQueryFlow() {
  queryFlowHasFailure.value = true
  const now = Date.now()
  for (const activity of queryFlowActivities.value) {
    if (activity.id !== 'answer' && activity.status !== 'completed' && activity.status !== 'failed') {
...
function handleToolCallActivity(t: { toolName: string; status: 'running' | 'completed' | 'failed'; content?: string }) {
  ...
  if (!label) return
  if (t.status === 'failed' || (t.status === 'completed' && isFailedToolResult(t.content))) {
    failQueryFlow()
    const failed = queryFlowActivities.value.find((activity) => activity.name === label)
    if (failed) {
      failed.status = 'failed'
      failed.elapsed = Date.now() - failed.startTime
    }
    return
  }
  if (t.toolName === 'read-cypher' && t.status === 'completed' && !parseNeo4jReadCypherGraph(t.content ?? '')) {
    failQueryFlow()
    return
  }
  if (t.toolName === 'evidence_subgraph' && t.status === 'completed') {
    const envelope = parseGraphifyEvidence('', t.content ?? '')
    const nodeKinds = new Set(['product', 'registration', 'organization', 'base', 'concept', 'attribute', 'catalog_record', 'source_file', 'import_batch', 'entity', 'record', 'source'])
  ...
  if (!step) {
    ...
    // 事件乱序：已完成的步骤不允许被迟到的 running 事件打回处理中。
    if (t.status === 'running' && step.status === 'completed') return
...
function completeAnswerComposition() {
  const now = Date.now()
  if (queryFlowHasFailure.value) {
    // 失败轮次只把未执行的查询阶段标为失败；“整理回答”仍可正常完成错误说明。
    for (const activity of queryFlowActivities.value) {
      if (activity.id !== 'answer' && activity.status !== 'completed' && activity.status !== 'failed') {
        activity.status = 'failed'
        activity.elapsed = now - activity.startTime
    ...
    // 正常结束时，若流事件缺失，按已进入回答阶段收口前置步骤，不能留下“等待”。
    settleEarlierSteps('整理回答')
  }
  const answer = queryFlowActivities.value.find((activity) => activity.id === 'answer')
  if (answer?.status === 'running') {
    answer.status = 'completed'
    answer.elapsed = now - answer.startTime
  } else if (answer?.status === 'pending') {
    answer.status = queryFlowHasFailure.value ? 'failed' : 'completed'
    answer.elapsed = now - answer.startTime
  }
...
function handleRunStateChanged(isRunning: boolean) {
  if (isRunning) {
    ...
    queryFlowHasFailure.value = false
  } else {
    completeAnswerComposition()
  }
}

/** 仅将南网问数的工具过程映射为业务步骤，状态卡仍由 Chat 消息流就近渲染。 */
...
const PROCESS_NARRATION = /trace_id|语义上下文|构造\s*SQL|现在(?:开始)?(?:执行|构造|查询)|(?:执行|进行)\s*preflight|\bSELECT\b|\bFROM\s+MOCK_APP/i
...
<template>
  ...
  <main v-else class="graphify-route-state" aria-live="polite">
    <ASpin v-if="loading" tip="正在加载南网问数助手…" />
    <section v-else>
      <p>{{ loadError }}</p>
