...
test('南网问数路由 enables a route-scoped full-page Graph Explorer modal', () => {
...
test('Graph Explorer modal embeds the complete external page in an iframe', () => {
  assert.match(modal, /resolveGraphExplorerUrl\(import\.meta\.env\.VITE_NANWANG_GRAPH_EXPLORER_URL\)/)
  assert.doesNotMatch(modal, /VITE_GRAPH_EXPLORER_URL/)
  assert.doesNotMatch(modal, /graph-explorer-modal-header/)
  assert.match(modal, /<iframe[\s\S]*:src="graphExplorerUrl"[\s\S]*title="数据管理"/)
