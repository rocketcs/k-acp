...
SELECT IF(COUNT(*)=1,'OK prompt requires source links','FAIL prompt requires source links') FROM system_prompt_template sp JOIN agent_definition a ON a.system_prompt_template_id=sp.id WHERE a.agent_code='default-tender' AND sp.content LIKE '%必须先调用 `resolve_tender_source_urls_v2`%' AND sp.content LIKE '%display_url%';
SELECT IF(COUNT(*)=1,'OK tender skill requires source links','FAIL tender skill requires source links') FROM skill_file sf JOIN skill_package sp ON sp.id=sf.skill_id WHERE sp.name='tender-search' AND sf.file_path='SKILL.md' AND sf.content LIKE '%这是输出前置条件%' AND sf.content LIKE '%标题必须写为 `[项目标题](display_url)`%';
