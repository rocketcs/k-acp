...
- The workspace already has unrelated uncommitted changes under `ui/src/features/graphify-data-query/`; preserve them and stage only files named in each task.
...
```bash
git add ui/src/features/graphify-data-query/evidenceGraphModel.ts ui/src/features/graphify-data-query/evidenceGraphModel.test.ts ui/src/features/graphify-data-query/GraphifyDataQueryPage.vue
