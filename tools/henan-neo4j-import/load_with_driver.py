...
QUERIES = {
 ...
 'catalog_attribute_values': "UNWIND $rows AS row MATCH (record:CatalogRecord {record_id:row.record_id}) MERGE (a:CatalogAttributeValue {attribute_id:row.attribute_id}) ON CREATE SET a.record_id=row.record_id,a.domain=row.domain,a.field=row.field,a.field_label=row.field_label,a.value=row.value,a.created_at=datetime() SET a.domain=row.domain,a.field=row.field,a.field_label=row.field_label,a.value=row.value MERGE (record)-[:HAS_ATTRIBUTE {field:row.field,field_label:row.field_label}]->(a)",
}
ORDER = ['consumable_categories','service_categories','service_items','diagnosis_items','mapping_concepts','consumable_bases','consumable_products','consumable_product_evidence','basic_materials','consumable_skus','mapping_labels','mapping_assertions','mapping_basic','concept_mentions','drug_products','catalog_attribute_values']
